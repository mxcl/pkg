...
pub(super) fn post_install(prefix: &Path, bin_dir: &Path) -> Result<PostInstallOutcome, String> {
    ...
    write_dispatch_script(&bin_dir.join("python"), &installed, "python")?;
    write_dispatch_script(&bin_dir.join("pip"), &installed, "pip")?;
    write_symlink(
...
fn write_dispatch_script(
    path: &Path,
    installed: &[InstalledPython],
    command: &str,
) -> Result<(), String> {
    let candidates = installed.iter().map(|python| {
        if command == "python" {
            python.python_stub.as_path()
        } else {
            python.pip_stub.as_path()
        }
    });

    let mut script = super::super::stub_script_prelude("# generated by p0r python dispatcher");
    script.push_str("for candidate in");
    for candidate in candidates {
        script.push(' ');
        script.push_str(&shell_quote(candidate.to_string_lossy().as_ref()));
    }
    script.push_str(";\ndo\n");
    script.push_str("  if [ -x \"$candidate\" ]; then\n");
    script.push_str("    exec \"$candidate\" \"$@\"\n");
    script.push_str("  fi\n");
    script.push_str("done\n");
    script.push_str(&format!(
        "echo \"no installed {command} found\" >&2\nexit 1\n"
    ));

    fs::write(path, script).map_err(|err| format!("failed to write {}: {err}", path.display()))?;
