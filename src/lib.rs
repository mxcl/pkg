...
#[cfg(feature = "gold-release")]
const SELF_UPDATE_TARGET: &str = "/usr/local/bin/subs";
...
const STUB_HEADER: &str = "# generated by subs";
...
fn resolve_self_update_release() -> Result<Option<SelfUpdateRelease>, String> {
    ...
    let current_version = semver::Version::parse(env!("CARGO_PKG_VERSION"))
        .map_err(|err| format!("failed to parse current subs version: {err}"))?;
    ...
    let asset = release
        ...
        .ok_or_else(|| {
            format!(
                "latest subs release {} does not contain asset {}",
...
fn install_self_update(release: &SelfUpdateRelease) -> Result<(), String> {
    ...
    download_vendor_asset(&release.download_url, &archive_path, "subs", None)?;
    unpack_bottle(&archive_path, temp_dir.path())?;

    let extracted = temp_dir.path().join("subs");
    let metadata = fs::metadata(&extracted)
        .map_err(|err| format!("failed to stat {}: {err}", extracted.display()))?;
    if !metadata.is_file() {
        return Err(format!(
            "self-update archive for subs {} did not contain a subs binary",
            release.version
        ));
    }
    ...
    fs::rename(&extracted, target).map_err(|err| {
        format!(
            "failed to replace {} with subs {}: {err}",
            target.display(),
            release.version
        )
    })
...
or `npm:clawhub` for the aliased package"
    ...
    fn mode_from_name_accepts_subcommands_and_aliases() {
        ...
        assert_eq!(Mode::from_name("subs"), None);
    }

    #[test]
    fn invocation_from_program_uses_direct_mode_for_renamed_entrypoints() {
        let subs = Invocation::from_program(&OsString::from("subs"));
        assert_eq!(subs.binary_name, "subs");
        assert_eq!(subs.name, "subs");
        assert_eq!(subs.mode, None);
    ...
    fn invocation_for_subcommand_uses_requested_alias_in_display_name() {
        ...
        assert_eq!(invocation.binary_name, "subs");
        assert_eq!(invocation.name, "subs x");
