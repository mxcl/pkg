...
#[cfg(feature = "gold-release")]
const SELF_UPDATE_TARGET: &str = "/usr/local/bin/p0r";
const SELF_UPDATE_DISABLE_FLAG: &str = "--no-self-update";
#[cfg(feature = "gold-release")]
const SELF_UPDATE_REPO: &str = "mxcl/the-package-managinat0r";
...
const STUB_HEADER: &str = "# generated by p0r";
...
pub fn main_entry() {
    ...
    let program = args.next().unwrap_or_else(|| OsString::from("p0r"));
...
fn resolve_self_update_release() -> Result<Option<SelfUpdateRelease>, String> {
    ...
    let current_version = semver::Version::parse(env!("CARGO_PKG_VERSION"))
        .map_err(|err| format!("failed to parse current p0r version: {err}"))?;
    ...
    let asset = release
        ...
        .ok_or_else(|| {
            format!(
                "latest p0r release {} does not contain asset {}",
...
fn self_update_asset_name_for(version: &semver::Version, os: &str, arch: &str) -> Option<String> {
    ...
    Some(format!(
        "ThePackageManaginat0r-{version}-{os}-{arch}.tar.gz"
    ))
}
...
fn install_self_update(release: &SelfUpdateRelease) -> Result<(), String> {
    ...
    download_vendor_asset(&release.download_url, &archive_path, "p0r", None)?;
    unpack_bottle(&archive_path, temp_dir.path())?;

    let extracted = temp_dir.path().join("p0r");
    let metadata = fs::metadata(&extracted)
        .map_err(|err| format!("failed to stat {}: {err}", extracted.display()))?;
    if !metadata.is_file() {
        return Err(format!(
            "self-update archive for p0r {} did not contain a p0r binary",
            release.version
        ));
    }
    ...
    fs::rename(&extracted, target).map_err(|err| {
        format!(
            "failed to replace {} with p0r {}: {err}",
            target.display(),
            release.version
        )
    })
...
package or `bar` for the package that provides the `foo` executable"
    ...
    fn mode_from_name_accepts_subcommands_and_aliases() {
        ...
        assert_eq!(Mode::from_name("p0r"), None);
    }

    #[test]
    fn invocation_from_program_uses_direct_mode_for_renamed_entrypoints() {
        let p0r = Invocation::from_program(&OsString::from("p0r"));
        assert_eq!(p0r.binary_name, "p0r");
        assert_eq!(p0r.name, "p0r");
        assert_eq!(p0r.mode, None);
    ...
    fn invocation_for_subcommand_uses_requested_alias_in_display_name() {
        ...
        assert_eq!(invocation.binary_name, "p0r");
        assert_eq!(invocation.name, "p0r x");
    ...
    fn self_update_asset_name_for_uses_release_naming() {
        ...
        assert_eq!(
            ...
            Some("ThePackageManaginat0r-0.1.0-Darwin-arm64.tar.gz".to_string())
        );
        assert_eq!(
            self_update_asset_name_for(&version, "linux", "x86_64"),
            Some("ThePackageManaginat0r-0.1.0-Linux-x86_64.tar.gz".to_string())
        );
        assert_eq!(
